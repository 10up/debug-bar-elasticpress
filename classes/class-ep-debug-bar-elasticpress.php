...
class EP_Debug_Bar_ElasticPress extends Debug_Bar_Panel {
	...
	public function render() {
		...
		<h2><?php printf( __( '<span>Total ElasticPress Query Time:</span> %d ms', 'debug-bar' ), (int) ( $total_query_time * 1000 ) ); ?></h2>
		...
		<?php else : ?>
			<ol class="wpd-queries">
				<?php foreach ( $queries as $query ) : $query_time = ( ! empty( $query['time_start'] ) && ! empty( $query['time_finish'] ) ) ? $query['time_finish'] - $query['time_start'] : false; ?>
					<li>
						<strong><?php esc_html_e( 'Host:', 'debug-bar'); ?></strong> <?php esc_url( $query['host'] ); ?><br>
						...
						<?php endif; ?>
						<strong><?php esc_html_e( 'URL:', 'debug-bar'); ?></strong> <?php esc_url( $query['url'] ); ?><br>
						<strong><?php esc_html_e( 'Query Arguments:', 'debug-bar'); ?></strong> <pre><?php print_r( $query['args'] ); ?></pre><br>
					</li>
				<?php endforeach; ?>
			</ol>
		<?php endif; ?>
